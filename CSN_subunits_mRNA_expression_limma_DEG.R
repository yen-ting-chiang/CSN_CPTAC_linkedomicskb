## =========================================================================
##  CSN Subunits mRNA Expression Limma DEG Analysis
...
##    Differentially Expressed Gene (DEG) analysis at the mRNA level.
##    The predictor (independent variable) is protein abundance, while the
##    response (dependent variable) is mRNA expression (RNAseq).
##    Additionally includes CSN_SCORE (PCA PC1 of z-scored CSN subunit
##    protein abundances, with COPS7A/7B combined) as a composite predictor,
##    and resid_* predictors (individual subunit with CSN_SCORE partialled
##    out as an additional covariate) to isolate each subunit's unique
##    contribution.
##
##  Key difference from protein-level DEG analysis:
##    - Predictor: protein abundance (same as template)
##    - Response:  mRNA expression (RNAseq RSEM UQ log2-transformed)
##    This tests whether protein-level variation of each CSN subunit
##    associates with genome-wide mRNA expression changes.
##
##  Statistical rationale:
##    Using a continuous predictor in limma is a well-established approach
##    (limma User's Guide Section 9.4). It identifies genes whose mRNA
##    expression changes linearly with the CSN subunit protein level,
...
##  mRNA data: NOT imputed (genes with excessive missing values are
...
OUT_ROOT <- file.path(BASE_DIR, "CSN_subunits_mRNA_expression_limma_DEG")
...
# --- Output toggle flags ---
# Set to TRUE to enable saving the corresponding combined CSV files.
# By default, only per-predictor CSV files are always saved.

# Save per-dataset combined CSV (all predictors for one dataset+stratum)
# e.g., BRCA_TP53_all_limma_DEG_all_predictors.csv
SAVE_PER_DATASET_COMBINED <- FALSE

# Save per-stratum combined CSV (all datasets for one stratum)
# e.g., ALL_datasets_TP53_all_limma_DEG_all_predictors.csv
SAVE_PER_STRATUM_COMBINED <- FALSE

# Save grand combined CSV (all datasets x all strata together)
# e.g., ALL_datasets_ALL_strata_limma_DEG_all_predictors.csv
SAVE_GRAND_COMBINED <- FALSE
...
# ---- 5. Main function: limma mRNA-level DEG for one dataset + stratum ----

run_limma_mrna_deg_one_dataset <- function(
    ds_id,
    stratum       = "TP53_all",
    sample_subset = NULL,
    tp53_lists    = NULL,
  ...
  # --- 5a. Read proteomics matrix (for predictors) --------------------------
  #     Protein abundance is used as the predictor (independent variable)
  ...
  if (!file.exists(prot_file)) {
    message(sprintf("[%s|%s] Proteomics file not found (for predictors): %s",
  ...
  # Build numeric matrix for protein data (genes x samples)
  prot_mat <- as.matrix(prot_dt[, ..prot_sample_ids])
  rownames(prot_mat) <- prot_ensembl_ids_raw
  storage.mode(prot_mat) <- "double"

  message(sprintf("[%s|%s] Proteomics matrix (for predictors): %d genes x %d samples",
                  ds_id, stratum, nrow(prot_mat), ncol(prot_mat)))

  # --- 5a2. Read mRNA expression matrix (response variable) -----------------
  #      mRNA expression is used as the response (dependent variable)

  mrna_file <- file.path(ds_dir,
    paste0(ds_id,
           "_RNAseq_gene_RSEM_coding_UQ_1500_log2_Tumor.txt"))

  if (!file.exists(mrna_file)) {
    message(sprintf("[%s|%s] mRNA expression file not found (for response): %s",
                    ds_id, stratum, basename(mrna_file)))
    return(invisible(NULL))
  }

  mrna_dt <- data.table::fread(mrna_file, header = TRUE, check.names = FALSE)
  mrna_idx_col <- names(mrna_dt)[1]  # "idx"
  mrna_ensembl_ids_raw <- mrna_dt[[mrna_idx_col]]

  # Strip version suffix from Ensembl IDs
  mrna_ensembl_ids_base <- sub("\\.[0-9]+$", "", mrna_ensembl_ids_raw)

  # Map ALL mRNA Ensembl IDs to gene symbols
  mrna_gene_symbols <- ens2sym[mrna_ensembl_ids_base]
  # For unmapped Ensembl IDs, keep the raw Ensembl ID as identifier
  mrna_gene_symbols[is.na(mrna_gene_symbols)] <- mrna_ensembl_ids_raw[is.na(mrna_gene_symbols)]
  names(mrna_gene_symbols) <- mrna_ensembl_ids_raw

  # Sample IDs from mRNA data
  mrna_sample_ids <- setdiff(names(mrna_dt), mrna_idx_col)

  # Build numeric matrix for mRNA data (genes x samples)
  mrna_mat <- as.matrix(mrna_dt[, ..mrna_sample_ids])
  rownames(mrna_mat) <- mrna_ensembl_ids_raw
  storage.mode(mrna_mat) <- "double"

  message(sprintf("[%s|%s] mRNA expression matrix (for response): %d genes x %d samples",
                  ds_id, stratum, nrow(mrna_mat), ncol(mrna_mat)))

  # --- 5a3. Find common samples between proteomics and mRNA data -----------
  #      Only samples present in BOTH datasets can be used

  common_samples <- intersect(prot_sample_ids, mrna_sample_ids)
  message(sprintf("[%s|%s] Common samples between proteomics and mRNA: %d (prot=%d, mRNA=%d)",
                  ds_id, stratum, length(common_samples),
                  length(prot_sample_ids), length(mrna_sample_ids)))

  if (length(common_samples) < 10) {
    message(sprintf("[%s|%s] Too few common samples (%d, need >= 10), skip",
                    ds_id, stratum, length(common_samples)))
    return(invisible(NULL))
  }
  ...
  if (!is.null(sample_subset)) {
    ...
    prot_mat <- prot_mat[, sam_stratum, drop = FALSE]
    mrna_mat <- mrna_mat[, sam_stratum, drop = FALSE]
    all_sample_ids <- sam_stratum
  }

  message(sprintf("[%s|%s] Final sample count after all filtering: %d",
                  ds_id, stratum, length(all_sample_ids)))

  # --- 5b. Identify CSN subunit rows in the PROTEOMICS matrix ---------------
  #     (predictors come from protein abundance)
  ...
  message(sprintf("[%s|%s] Found %d / %d CSN subunits in proteomics: %s",
  ...
  # (uses PROTEIN abundance for CSN_SCORE computation)
  present_sub <- names(csn_row_map)
  mat0_csn <- prot_mat[unlist(csn_row_map), , drop = FALSE]
  ...
  #     (all predictors come from PROTEIN abundance)

  # Compute CSN_SCORE via PCA PC1 of z-scored CSN subunit protein abundances
  ...
  message(sprintf("[%s|%s] CSN_SCORE computed (from protein): %d / %d samples with valid score",
                  ds_id, stratum, csn_score_valid, length(csn_score_vec)))

  # Build predictor list: each individual CSN subunit protein abundance + CSN_SCORE
  predictor_list <- list()
  for (sub in present_sub) {
    v <- as.numeric(prot_mat[csn_row_map[[sub]], ])
  ...
  # Add resid_* predictors: same subunit protein abundance as predictor, but
  # with CSN_SCORE added as an additional covariate to partial out the shared
  ...
  #     Predictor: protein abundance; Response: mRNA expression

  all_deg_results <- list()

  for (pred_name in names(predictor_list)) {

    message(sprintf("\n  [%s|%s | %s] Running limma mRNA-level DEG analysis ...",
    ...
    # Extract predictor vector (protein abundance)
    ...
    # Subset mRNA expression matrix (response) and predictor to valid samples
    mrna_sub <- mrna_mat[, sam_use, drop = FALSE]
    pred_sub <- pred_vec[sam_use]

    # Filter mRNA genes: require sufficient non-NA values across valid samples
    n_nonmissing <- rowSums(!is.na(mrna_sub))
    threshold    <- max(min_n_gene, ceiling(length(sam_use) * min_prop))
    gene_keep    <- n_nonmissing >= threshold
    mrna_sub     <- mrna_sub[gene_keep, , drop = FALSE]

    message(sprintf("  [%s|%s | %s] mRNA genes retained (>= %d non-NA values): %d / %d",
                    ds_id, stratum, pred_name, threshold,
                    sum(gene_keep), length(gene_keep)))

    if (nrow(mrna_sub) == 0) {
      message(sprintf("  [%s|%s | %s] No mRNA genes passed the filter, skip",
                      ds_id, stratum, pred_name))
      next
    }
    ...
    # Run limma: lmFit + eBayes (response = mRNA expression)
    fit <- limma::lmFit(mrna_sub, design_mat)
    ...
    # Use mRNA Ensembl IDs and gene symbols for the response genes
    matched_ensembl_raw  <- rownames(tt)
    matched_symbols      <- mrna_gene_symbols[matched_ensembl_raw]
    ...
    #   Note: is_predictor flags mRNA genes that correspond to the protein
    #   predictor(s), i.e., the mRNA of the CSN subunit(s) used as predictor
    ...
    message(sprintf(
      "  [%s|%s | %s] Results: %d mRNA genes tested | BH FDR < 0.05: %d | < 0.1: %d | < 0.25: %d",
  ...
  if (length(all_deg_results) > 0) {
    ...
    if (SAVE_PER_DATASET_COMBINED) {
      ...
      message(sprintf("\n[%s|%s] Per-dataset combined CSV skipped (SAVE_PER_DATASET_COMBINED = FALSE)",
                      ds_id, stratum))
    }
...
message("\n============================================================")
message("  CSN Subunits mRNA Expression Limma DEG Analysis")
message("  Predictors: each CSN subunit's PROTEIN abundance + CSN_SCORE (continuous)")
message("  Response:   mRNA expression (RNAseq RSEM UQ log2)")
...
for (ds in DATASETS) {
  ...
  for (st in strata_to_run) {
    ...
    if (st == "TP53_all") {
      sample_sub <- NULL  # use all samples in data
    ...
    res <- tryCatch(
      run_limma_mrna_deg_one_dataset(
        ds_id         = ds,
        stratum       = st,
...
if (length(all_results) > 0) {
  ...
  if (SAVE_PER_STRATUM_COMBINED) {
    ...
    message("Per-stratum combined CSV skipped (SAVE_PER_STRATUM_COMBINED = FALSE)")
  }

  # --- Grand combined file (all strata together) ---
  if (SAVE_GRAND_COMBINED) {
    ...
    message("Grand combined CSV skipped (SAVE_GRAND_COMBINED = FALSE)")
  }
  ...
  message("\n========== mRNA-level DEG Analysis Summary (excluding self-predictor) ==========")
...
message("\n============================================================")
message("  All mRNA-level DEG analyses completed!")
