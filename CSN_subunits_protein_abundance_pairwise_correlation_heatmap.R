## =========================================================================
##  CSN Subunits Protein Abundance Pairwise Correlation & Heatmap
...
OUT_ROOT  <- file.path(BASE_DIR, "CSN_subunits_protein_abundance_pairwise_correlation")
...
# ---- 4. Helper: residualize protein abundance to covariates --------------

residualize_to_covars <- function(y, covars = NULL, min_n = 8L) {
  ## y: named numeric vector (sample -> abundance)
...
csn_pairwise_correlation_one_ds <- function(
  ...
  # --- 6a. Read proteomics matrix -----------------------------------------
  prot_file <- file.path(ds_dir,
    paste0(ds_id, "_proteomics_gene_abundance_log2_reference_intensity_normalized_Tumor.txt"))
  ...
  if (length(hit_idx) == 0) {
    message(sprintf("[%s|%s] No CSN subunit Ensembl IDs found in proteomics data",
                    ds_id, stratum))
    return(invisible(NULL))
  }
  ...
  mat0 <- as.matrix(prot_dt[hit_idx, ..all_sample_ids])
  ...
  if (!is.null(sample_subset)) {
    # Intersect with samples actually present in proteomics data
...
message("\n============================================================")
message("  Starting CSN subunits pairwise correlation analysis")
message("  with TP53 mutation status stratification")
message("============================================================\n")
...
message("\n============================================================")
message("  Generating correlation coefficient heatmaps")
  ...
  for (st in strata_to_plot) {
    ...
    for (ver in versions) {
      ...
      if (!is.null(grid_result)) {
        title_str <- sprintf("%s | CSN Subunits Protein Abundance\nPearson r (%s, %s)",
                             ds, st, ver)
