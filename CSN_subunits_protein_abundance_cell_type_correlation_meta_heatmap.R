## =========================================================================
##  CSN Subunits Protein Abundance vs Cell Type Correlation Meta Heatmap
##  Data source: Meta-analysis results from
##               CSN_subunits_protein_abundance_cell_type_correlation_meta.R
##
##  Purpose:
##    Produce per-prefix, per-stratum heatmaps of cross-dataset
...
##            stouffer_Z_score from largest to smallest)
##    Tile fill color: stouffer_Z_score
##    Significance marker: "*" where stouffer_BH_FDR < 0.05
##
##  x-axis predictor ordering and gap rules follow the same convention
##  as the GSEA heatmaps in the companion BRCA immune analysis script
##  (CSN_CPTAC_BRCA_immune_analysis_V20251022_2_add_heatmap_3_en_opus_3_cleanup.R).
##
##  Output structure:
##    CSN_subunits_protein_abundance_cell_type_correlation_meta_heatmap/
##      {TP53_all, TP53_MUT, TP53_WT}/
##        {CIBERSORT, ESTIMATE, xCell}/
##          {stratum}_{prefix}_meta_correlation_heatmap.tiff
##          {stratum}_{prefix}_meta_correlation_heatmap_data.csv
## =========================================================================
...
INPUT_DIR <- file.path(BASE_DIR, "CSN_subunits_protein_abundance_cell_type_correlation_meta")
OUT_ROOT  <- file.path(BASE_DIR, "CSN_subunits_protein_abundance_cell_type_correlation_meta_heatmap")
...
# ---- 3. x-axis predictor ordering (same as GSEA heatmaps) ---------------
...
# ---- 4. Scan and load meta-analysis result files -------------------------

message("Scanning for meta-analysis result files in: ", INPUT_DIR)
all_files <- list.files(
  INPUT_DIR,
  pattern    = "_meta_.*_correlation_predictor_.*\\.csv$",
  recursive  = TRUE,
  full.names = TRUE
)

if (length(all_files) == 0) {
  stop("No meta-analysis result files found in: ", INPUT_DIR)
}
...
req_cols <- c("stratum", "prefix", "predictor",
              "cell_type", "stouffer_Z_score", "stouffer_BH_FDR")
...
plot_correlation_meta_heatmap <- function(
    plot_dt,
    stratum,
    prefix,
    out_dir,
  ...
  if (nrow(plot_dt) == 0) {
    message(sprintf("  [%s|%s] No data, skip heatmap", stratum, prefix))
  ...
  if (length(present_preds) == 0) {
    message(sprintf("  [%s|%s] No matching predictors, skip", stratum, prefix))
    return(invisible(NULL))
  }
  plot_dt <- plot_dt[predictor %in% present_preds]

  # --- y-axis: cell_type ordered by CSN_SCORE's stouffer_Z_score (desc) ---
  csn_score_dt <- plot_dt[predictor == "CSN_SCORE"]
  if (nrow(csn_score_dt) > 0) {
    y_order <- csn_score_dt[order(-stouffer_Z_score)]$cell_type
  } else {
    # Fallback: use overall mean stouffer_Z_score across predictors
    mean_z <- plot_dt[, .(mean_z = mean(stouffer_Z_score, na.rm = TRUE)),
                      by = cell_type]
    y_order <- mean_z[order(-mean_z)]$cell_type
    message(sprintf("  [%s|%s] CSN_SCORE not found, using mean Z-score for y-order",
                    stratum, prefix))
  }
  ...
  L <- max(abs(plot_dt$stouffer_Z_score), na.rm = TRUE)
  if (!is.finite(L) || L == 0) L <- 1

  # --- Build heatmap ---
  g <- ggplot2::ggplot(
    plot_dt,
    ggplot2::aes(x = xpos, y = cell_type, fill = stouffer_Z_score)
  ) +
    ggplot2::geom_tile(width = 1, height = 0.9, color = NA) +
    # Significance marker: "*" for stouffer_BH_FDR < 0.05
    ggplot2::geom_text(
      data = plot_dt[is.finite(stouffer_BH_FDR) & stouffer_BH_FDR < 0.05],
    ...
    ggplot2::labs(
      x = NULL, y = NULL,
      title = sprintf("CSN Subunit Protein vs %s (Meta-analysis)", prefix),
      subtitle = sprintf("Stratum: %s | Color: Stouffer Z-score | * FDR < 0.05",
                          stratum)
    ) +
  ...
  # --- Save heatmap ---
  base_bn <- paste0(stratum, "_", prefix, "_meta_correlation_heatmap")
  ...
  for (f in fmts) {
    ...
    if (f == "TIFF") {
      tryCatch({
        ...
        invisible(print(g))
        grDevices::dev.off()
        message(sprintf("  [%s|%s] Saved: %s", stratum, prefix, basename(target)))
        any_ok <- TRUE
      }, error = function(e) {
        message(sprintf("  [%s|%s] TIFF save failed: %s", stratum, prefix,
                        conditionMessage(e)))
      ...
      tryCatch({
        ...
        message(sprintf("  [%s|%s] %s save failed: %s", stratum, prefix, f,
                        conditionMessage(e)))
      })
    }
  }
  ...
  if (!any_ok) {
    message(sprintf("  [%s|%s] WARNING: No output files generated", stratum, prefix))
  }
  ...
  message(sprintf("  [%s|%s] Data CSV saved: %s", stratum, prefix, basename(csv_out)))

  invisible(list(plot = g, width = W, height = H))
}

# ---- 7. Main loop: iterate over strata and prefixes ---------------------

message("\n============================================================")
message("  CSN Subunits Protein Abundance vs Cell Type")
message("  Correlation Meta-Analysis Heatmaps")
message("  Color: Stouffer Z-score | * : Stouffer BH FDR < 0.05")
message("============================================================\n")
...
for (strat in strata) {
  for (pfx in prefixes) {
    ...
    if (nrow(sub_dt) == 0) {
      message(sprintf("  [%s|%s] No data found, skip", strat, pfx))
      next
    }
    ...
    tryCatch(
      ...
      error = function(e) {
        message(sprintf("  [%s|%s] ERROR: %s", strat, pfx, conditionMessage(e)))
      }
    )
  }
}
...
message("\n============================================================")
message("  All heatmaps completed!")
