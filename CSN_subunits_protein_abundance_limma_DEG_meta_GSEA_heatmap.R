## =========================================================================
##  CSN Subunits Protein Abundance limma DEG Meta-Analysis GSEA Heatmaps
##  Data source: Meta-analysis output from CSN_subunits_protein_abundance_limma_DEG_meta_GSEA
##
##  Purpose:
...
INPUT_DIR <- file.path(BASE_DIR, "CSN_subunits_protein_abundance_limma_DEG_meta_GSEA")
OUT_ROOT <- file.path(BASE_DIR, "CSN_subunits_protein_abundance_limma_DEG_meta_GSEA_heatmap")
...
.meta_make_heatmap_plot <- function(df_long, coll_tok, y_order = NULL,
  ...
  if (is.null(y_order)) {
    if (!identical(toupper(coll_tok), "H") && "NES_CSN_SCORE" %in% present) {
      .top_n <- 25
      .bot_n <- 25
