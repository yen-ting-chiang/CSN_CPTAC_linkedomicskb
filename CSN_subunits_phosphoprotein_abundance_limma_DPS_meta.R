## =========================================================================
##  CSN Subunits Phosphoprotein Abundance Limma DPS Meta-Analysis
##  Data source: Limma DPS results from CPTAC datasets
##
##  Purpose:
##    Perform cross-dataset meta-analysis using Stouffer's z-score method 
##    with Benjamini-Hochberg FDR correction.
##    This script reads the output from CSN_subunits_phosphoprotein_abundance_limma_DPS.R
...
message("Scanning for Limma DPS result files in: ", INPUT_DIR)
all_files <- list.files(INPUT_DIR, pattern = "_limma_DPS_predictor_.*\\.csv$", recursive = TRUE, full.names = TRUE)

if (length(all_files) == 0) {
  stop("No Limma DPS result files found in the input directory.")
}
...
req_cols <- c("dataset", "stratum", "csn_subunit", "phospho_site_id", "ensembl_gene_id", 
              "ensembl_gene_base", "gene_symbol", "phospho_site", "logFC", "p_value", 
              "n_samples", "is_predictor")
...
# ---- 4. Execute meta-analysis per stratum, per predictor, per phosphosite -
...
for (strat in strata) {
  ...
  for (pred in predictors) {
    ...
    # Group by phosphosite and apply meta-analysis
    meta_res <- pred_dt[, run_stouffer_meta(.SD), by = .(phospho_site_id, ensembl_gene_id, ensembl_gene_base, gene_symbol, phospho_site)]
    
    # Filter by minimum datasets
    meta_res <- meta_res[n_datasets >= MIN_DATASETS]
    
    if (nrow(meta_res) == 0) {
      message(sprintf("  No phosphosites passed the MIN_DATASETS >= %d filter. Skipping.", MIN_DATASETS))
      next
    }
    ...
    setcolorder(meta_res, c("phospho_site_id", "ensembl_gene_id", "ensembl_gene_base", 
                            "gene_symbol", "phospho_site",
    ...
    out_file <- file.path(strat_out_dir, paste0(strat, "_meta_limma_DPS_predictor_", pred, ".csv"))
    
    # Save to CSV
    data.table::fwrite(meta_res, out_file)
    
    n_sig_005 <- sum(meta_res$meta_BH_FDR < 0.05, na.rm = TRUE)
    message(sprintf("  Saved %s (Phosphosites: %d | FDR < 0.05: %d)", basename(out_file), nrow(meta_res), n_sig_005))
  }
}
